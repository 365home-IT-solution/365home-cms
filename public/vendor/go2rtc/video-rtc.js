 ...
 *
 * Vendored từ https://github.com/AlexxIT/go2rtc/blob/master/www/video-rtc.js (official go2rtc web
 * client) — giữ NGUYÊN VĂN phần giao thức MSE-qua-WebSocket (handshake JSON + fragment nhị phân)
 * thay vì tự viết lại, vì tự đoán/viết lại dễ sai lệch tinh vi (buffering, tốc độ phát, codec) mà
 * rất khó phát hiện qua test thủ công. CHỈ 1 chỗ chỉnh khác bản gốc: default của `background` (tìm
 * "ĐÃ CHỈNH" bên dưới) — mọi chỗ khác giữ nguyên logic gốc. File này chỉ chạy phía TRÌNH DUYỆT, load
 * bởi camera-monitor.blade.php, kết nối tới Node WebSocket proxy của CHÍNH DỰ ÁN NÀY
 * (websocket/server.js), KHÔNG kết nối thẳng tới Frigate. (Việc giảm tải khi mở nhiều camera cùng
 * lúc nằm ở camera-monitor.blade.php — rải so le thời điểm kết nối — không đụng tới file vendor
 * này.)
 *
...
export class VideoRTC extends HTMLElement {
    constructor() {
         ...
         * @type {boolean}
         *
         * ĐÃ CHỈNH true (bản gốc: false) — cùng lý do với comment ở visibilityThreshold từng có ở
         * đây: oninit()/disconnectedCallback() đọc giá trị này ngay lúc phần tử gắn vào DOM hoặc khi
         * tab bị ẩn, TRƯỚC/NGOÀI thời điểm x-init của Alpine kịp chạy — set qua x-init không có tác
         * dụng. Trang "Xem camera" là trang GIÁM SÁT: người dùng chuyển sang tab khác rồi quay lại
         * muốn camera VẪN ĐANG PHÁT (không mất vài giây kết nối lại mỗi lần), không phải trang xem
         * video giải trí nên không cần tự tạm dừng để tiết kiệm băng thông khi mất focus tab.
         */
         ...
         * @type {number}
         *
         * Giữ mặc định gốc (0 = tắt) — ĐÃ THỬ đặt 0.2 để giảm tải khi mở hết 17 camera cùng lúc,
         * nhưng người dùng phản hồi hành vi tự ngắt/kết nối lại theo cuộn trang gây khó chịu hơn
         * (đang xem thì mất hình, cuộn qua lại camera cứ load lại liên tục). Cách giảm tải hiện tại:
         * xem camera-monitor.blade.php — rải nhẹ thời điểm bắt đầu kết nối của từng camera (so le vài
         * trăm ms) thay vì bắt TẤT CẢ camera tự ngắt/nối lại theo việc cuộn trang.
         */
