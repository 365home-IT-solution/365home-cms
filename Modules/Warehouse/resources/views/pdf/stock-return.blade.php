...
<body>
    <h1>Phiếu hoàn trả kho</h1>
    <p class="subtitle">{{ $stockReturn->partner?->name ?? '—' }}</p>

    <table class="meta-table">
        <tr>
            <td class="meta-label">Số phiếu:</td>
            <td><strong>{{ $stockReturn->code }}</strong></td>
            <td class="meta-label">Ngày hoàn:</td>
            <td>{{ optional($stockReturn->returned_at)->format('d/m/Y H:i') }}</td>
        </tr>
        <tr>
            <td class="meta-label">Hoàn từ:</td>
            <td>{{ $stockReturn->employee?->name ?? $stockReturn->returned_by ?? '—' }}</td>
            <td class="meta-label">Người nhận hoàn:</td>
            <td>{{ $creatorName }}</td>
        </tr>
        @if ($stockReturn->room)
            <tr>
                <td class="meta-label">Phòng:</td>
                <td colspan="3">{{ $stockReturn->room->name }}</td>
            </tr>
        @endif
        @if ($stockReturn->note)
            <tr>
                <td class="meta-label">Ghi chú:</td>
                <td colspan="3">{{ $stockReturn->note }}</td>
            </tr>
        @endif
    </table>
    ...
    <table class="items">
        <thead>
            <tr>
                ...
                <th style="width: 70px;" class="num">SL hoàn</th>
                <th style="width: 130px;">Phiếu xuất gốc</th>
                <th>Ghi chú</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($stockReturn->items as $index => $line)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    ...
                    <td>{{ $line->stockOutItem?->stockOut?->code ?? '—' }}</td>
                    <td>{{ $line->note }}</td>
                </tr>
            @endforeach
        </tbody>
    ...
    <table class="signatures">
        <tr>
            <td class="sig-title">Người hoàn trả</td>
