...
class MinihouseZaloTokenService
{
    ...
    public function getAccessToken(bool $forceRefresh = false): string
    {
        if (! $forceRefresh) {
            ...
            // Token vừa bị Zalo từ chối (xem MinihouseZaloService::sendZns()) — xoá cache để không
            // ai khác đọc lại đúng token hỏng này trong lúc ta refresh, mirror App\Services\
            // ZaloTokenService::getAccessToken(forceRefresh).
            Cache::forget(self::CACHE_ACCESS_TOKEN);
        }
        ...
        return DB::transaction(function () use ($forceRefresh) {
            ...
            if (! $forceRefresh) {
                // Tiến trình khác có thể đã refresh xong TRONG LÚC ta chờ khoá — đọc lại Cache lần
                // nữa trước khi tự refresh thêm, tránh gọi Zalo thừa 1 lần cho access_token vẫn còn
                // hạn.
                $cached = Cache::get(self::CACHE_ACCESS_TOKEN);
