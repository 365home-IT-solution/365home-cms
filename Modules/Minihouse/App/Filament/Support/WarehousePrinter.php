...
use Modules\Minihouse\App\Support\WarehousePdfRenderer;
...
class WarehousePrinter
{
    ...
    {
        $stockIn->loadMissing(['items.item.unit', 'creator']);

        $html = view('minihouse::warehouse.pdf.stock-in', [
            'stockIn'     => $stockIn,
    ...
    {
        $stockOut->loadMissing(['items.item.unit', 'room', 'creator']);

        $html = view('minihouse::warehouse.pdf.stock-out', [
            'stockOut'    => $stockOut,
    ...
    {
        $stockReturn->loadMissing(['items.item.unit', 'items.stockOutItem.stockOut', 'room', 'creator']);

        $html = view('minihouse::warehouse.pdf.stock-return', [
    ...
    {
        $stockCheck->loadMissing(['items.item.unit', 'creator']);

        $html = view('minihouse::warehouse.pdf.stock-check', [
    ...
    {
        $items->loadMissing(['category', 'unit']);

        $partnerName = $showPartnerColumn ? null : null;

        $html = view('minihouse::warehouse.pdf.item-list', [
            'items'             => $items,
    ...
    public static function qrCodes(Collection $items, int $copies = 1): StreamedResponse
    {
        ...
        $view = $copies > 1 ? 'minihouse::warehouse.pdf.qr-stickers' : 'minihouse::warehouse.pdf.qr-codes';
