...
namespace Modules\Minihouse\App\Filament\Resources\WarehouseStockCheckResource\Forms;
...
class WarehouseStockCheckForm
{
                        ...
                        ->schema([
                            ...
                            Select::make('warehouse_item_id_picker')
                                ...
                                ->options(function (Get $get) {
                                    // $get(..., isAbsolute: true) để lấy đúng field GỐC của form (building_id),
                                    // không phải field cùng dòng lặp.
                                    $buildingId = $get('building_id', isAbsolute: true);

                                    return WarehouseItem::query()
                                        ->where('status', true)
                                        ->when($buildingId, fn ($query, $bid) => $query->where('building_id', $bid))
                                        ->orderBy('name')
                                        ->pluck('name', 'id');
                                })
                                ->searchable()
    ...
    public static function allActiveItemsAsRows(int|string|null $buildingId = null): array
    {
        return WarehouseItem::query()
            ->where('status', true)
            ->when($buildingId, fn ($query, $bid) => $query->where('building_id', $bid))
    ...
    private static function buildingInput(): Select
    {
        return Select::make('building_id')
            ...
            ->required()
            ->live()
            // Đổi Toà nhà giữa chừng — nạp lại TOÀN BỘ vật tư của toà mới vào phiếu (Repeater không tự
            // theo building_id), giống branchInput() của Home.
            ->afterStateUpdated(function ($state, Set $set, Get $get) {
                $items = [];

                foreach (WarehouseStockCheckForm::allActiveItemsAsRows($state) as $row) {
