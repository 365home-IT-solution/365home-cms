...
UserTable::table()), các cột gốc còn lại ->visibleFrom('md') — desktop không đổi 1 chữ nào. --}}
@php
    /** @var \App\Models\User $user */
    $user = $getRecord();
    $initials = collect(explode(' ', trim($user->fullname ?? '')))->map(fn ($w) => mb_substr($w, 0, 1))->slice(-2)->implode('');
...
<div x-data="{ expanded: false }" class="min-w-0 w-full">
    <div class="flex min-w-0 w-full items-center gap-2.5 py-0.5">
        ...
        <div class="min-w-0 flex-1">
            <div class="truncate text-[13px] font-semibold text-gray-950 dark:text-white">{{ $user->fullname }}</div>
            <div class="truncate text-[11.5px] text-gray-400 dark:text-gray-500">{{ $user->email ?: '—' }}</div>
        </div>

        <div class="flex-shrink-0 text-right">
            <div class="text-[12px] font-semibold text-gray-700 dark:text-gray-300">{{ $roleName ?? '—' }}</div>
    ...
    <div x-show="expanded" x-collapse x-cloak class="min-w-0 w-full rounded-lg bg-gray-50 p-2.5 text-[11.5px] dark:bg-white/5" style="margin-top: 4px;">
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Họ tên: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $user->fullname }}</span></div>
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Email: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $user->email ?: '—' }}</span></div>
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">SĐT: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $user->phone ?: '—' }}</span></div>
        <div><span class="text-gray-400 dark:text-gray-500">Vai trò: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $roleName ?? '—' }}</span></div>
    </div>
</div>
