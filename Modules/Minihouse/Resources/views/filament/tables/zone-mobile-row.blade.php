...
ZoneTable::table()), các cột gốc còn lại ->visibleFrom('md') — desktop không đổi 1 chữ nào. --}}
@php
    /** @var \Modules\Minihouse\App\Models\Zone $zone */
    $zone = $getRecord();
    $initials = mb_strtoupper(mb_substr(trim($zone->name ?? ''), 0, 2));
@endphp
...
<div x-data="{ expanded: false }" class="min-w-0 w-full">
    <div class="flex min-w-0 w-full items-center gap-2.5 py-0.5">
        ...
        <div class="min-w-0 flex-1">
            <div class="truncate text-[13px] font-semibold text-gray-950 dark:text-white">{{ $zone->name }}</div>
            <div class="truncate text-[11.5px] text-gray-400 dark:text-gray-500">{{ $zone->note ?: '—' }}</div>
        </div>

        <div class="flex-shrink-0 text-right">
            <div class="text-[12px] font-semibold text-gray-700 dark:text-gray-300">{{ $zone->buildings_count ?? $zone->buildings()->count() }} toà</div>
        </div>
    ...
    <div x-show="expanded" x-collapse x-cloak class="min-w-0 w-full rounded-lg bg-gray-50 p-2.5 text-[11.5px] dark:bg-white/5" style="margin-top: 4px;">
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Tên khu vực: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $zone->name }}</span></div>
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Ghi chú: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $zone->note ?: '—' }}</span></div>
        <div><span class="text-gray-400 dark:text-gray-500">Số toà nhà: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $zone->buildings_count ?? $zone->buildings()->count() }}</span></div>
    </div>
</div>
