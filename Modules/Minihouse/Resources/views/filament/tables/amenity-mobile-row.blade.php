...
AmenityTable::table()), các cột gốc còn lại ->visibleFrom('md') — desktop không đổi 1 chữ nào. --}}
@php
    /** @var \Modules\Minihouse\App\Models\Amenity $amenity */
    $amenity = $getRecord();
@endphp
...
<div x-data="{ expanded: false }" class="min-w-0 w-full">
    <div class="flex min-w-0 w-full items-center gap-2.5 py-0.5">
        <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
            @if ($amenity->image)
                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($amenity->image) }}" class="h-full w-full object-cover" alt="">
            @else
                <span class="text-[11px] font-semibold text-gray-500 dark:text-gray-400">{{ mb_strtoupper(mb_substr($amenity->name ?? '?', 0, 1)) }}</span>
            @endif
        </div>

        <div class="min-w-0 flex-1">
            <div class="truncate text-[13px] font-semibold text-gray-950 dark:text-white">{{ $amenity->name }}</div>
        </div>

        <div class="flex-shrink-0 text-right">
            <div class="text-[12px] font-semibold text-gray-700 dark:text-gray-300">{{ $amenity->rooms_count ?? 0 }} phòng</div>
        </div>
    ...
    <div x-show="expanded" x-collapse x-cloak class="min-w-0 w-full rounded-lg bg-gray-50 p-2.5 text-[11.5px] dark:bg-white/5" style="margin-top: 4px;">
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Tên tiện ích: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $amenity->name }}</span></div>
        <div><span class="text-gray-400 dark:text-gray-500">Số phòng dùng: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $amenity->rooms_count ?? 0 }}</span></div>
    </div>
</div>
