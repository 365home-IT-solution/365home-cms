...
AssetTypeTable::table()), các cột gốc còn lại ->visibleFrom('md') — desktop không đổi 1 chữ nào. --}}
@php
    /** @var \Modules\Minihouse\App\Models\AssetType $assetType */
    $assetType = $getRecord();
@endphp
...
<div x-data="{ expanded: false }" class="min-w-0 w-full">
    <div class="flex min-w-0 w-full items-center gap-2.5 py-0.5">
        <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-100 text-[11px] font-semibold text-gray-500 dark:bg-white/10 dark:text-gray-400">
            {{ mb_strtoupper(mb_substr($assetType->name ?? '?', 0, 1)) }}
        </div>

        <div class="min-w-0 flex-1">
            <div class="truncate text-[13px] font-semibold text-gray-950 dark:text-white">{{ $assetType->name }}</div>
            <div class="truncate text-[11.5px] text-gray-400 dark:text-gray-500">{{ $assetType->created_at?->format('d/m/Y') }}</div>
        </div>
    ...
    <div x-show="expanded" x-collapse x-cloak class="min-w-0 w-full rounded-lg bg-gray-50 p-2.5 text-[11.5px] dark:bg-white/5" style="margin-top: 4px;">
        <div style="margin-bottom: 5px;"><span class="text-gray-400 dark:text-gray-500">Tên loại tài sản: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $assetType->name }}</span></div>
        <div><span class="text-gray-400 dark:text-gray-500">Ngày tạo: </span><span class="font-medium text-gray-700 dark:text-gray-300">{{ $assetType->created_at?->format('d/m/Y H:i') }}</span></div>
    </div>
</div>
