<div
    ...
    class="relative -m-1.5 ml-2"
>
    <button
        type="button"
        x-ref="trigger"
        @click.stop="position(); open = ! open"
        title="Chuyển đổi toà nhà"
    ...
    >
        <x-heroicon-o-building-office-2 class="w-4 h-4 fi-icon-btn-icon" />
    </button>

    {{-- Teleport ra <body> — .fi-topbar có class overflow-x-clip, dropdown pop lên ngoài khung
         topbar sẽ bị cắt mất phần dưới nếu render ngay bên trong (đã xác nhận thực tế bên Home). --}}
    <template x-teleport="body">
        <div
            ...
            :style="`position: fixed; top: ${top}px; left: ${left}px; z-index: 99999; isolation: isolate;`"
            class="w-72 rounded-xl bg-white dark:bg-gray-900 shadow-lg ring-1 ring-gray-950/5 dark:ring-white/10 overflow-hidden"
        >
            <div class="p-3 border-b border-gray-100 dark:border-white/10 flex items-center justify-between">
                <p class="text-sm font-medium text-gray-950 dark:text-white">Chuyển đổi toà nhà</p>
            ...
            <div class="max-h-72 overflow-y-auto p-2 space-y-1">
                @forelse ($this->buildings() as $building)
                    <label class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer">
                        ...
                        />
                        <span class="text-sm text-gray-950 dark:text-white">{{ $building->name }}</span>
                    </label>
                @empty
                    <p class="px-2 py-4 text-sm text-gray-500 text-center">Chưa có toà nhà nào.</p>
                @endforelse
            </div>
