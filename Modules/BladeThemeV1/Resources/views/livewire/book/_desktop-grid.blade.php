                            ...
                            <div class="book-dt-slots-header-row" x-ref="bookDtHeaderRow{{ $loop->index }}"
                                ...
                                @foreach ($room->roomTimeSlots as $roomTimeSlot)
                                    ...
                                    <div class="book-dt-slot-th">
                                        ...
                                        <br>
                                        @if ($isOvernight)
                                            <svg class="w-4 h-4 inline" style="color:#1e3a8a" viewBox="0 0 24 24" fill="currentColor"><use href="#bk-moon"/></svg>
                                            <span class="text-xs" style="color:#1e3a8a">(Qua đêm)</span>
                                        @else
                                            <svg class="w-4 h-4 inline" style="color:#eab308" viewBox="0 0 16 16" fill="currentColor"><use href="#bk-sun"/></svg>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="book-dt-slots-card">
                                <div class="book-dt-slots-scroll" x-ref="bookDtSlotsScroll{{ $loop->index }}"
                                    ...
                                    {{-- Render đầy đủ (giá/khuyến mãi) cho MỌI phòng, kể cả các phòng
                                         "peek" 2 bên chưa active — đã verify chi phí tối đa (12 phòng
                                         x 31 ngày, chi nhánh nhiều phòng nhất hiện có) an toàn qua
                                         Livewire::test() (~100MB, memory_limit hiện tại 512M). Từng
                                         thử giới hạn chỉ render phòng active để tiết kiệm chi phí,
                                         nhưng gây lỗi ô trống khi đổi phòng sau khi đã "Xem thêm ngày"
                                         (activeRoomIndex phía server không đồng bộ kịp lúc đổi phòng
                                         thuần client-side) — ưu tiên đúng giao diện hơn tối ưu này. --}}
                                    @foreach ($dates as $date)
