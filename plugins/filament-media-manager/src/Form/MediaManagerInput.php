...
class MediaManagerInput extends Repeater
{
    ...
    protected function setUp(): void
    {
        ...
        $this->saveRelationshipsUsing(static function (Repeater $component, HasMedia $record): void {
            ...
            $getState = [];
            $mediasToDelete = $record->media()
                ->where('collection_name', $component->name)
                ->whereNotIn('uuid', $collectMediaIds)
                ->get();
            ...
            foreach ($mediasToDelete as $media) {
                try {
                    ...
                    Storage::disk($media->disk)->deleteDirectory($folderPath);

                    $media->delete();

                } catch (\Exception $e) {
                    Notification::make()
                        ->title(trans('filament-media-manager::messages.media.notifications.error.title'))
                        ->danger()
                        ->body(trans('filament-media-manager::messages.media.notifications.error.body'))
                        ->send();
                }
            }
        ...
        $this->reorderAction(static function (Action $action): void {
            $action->action(function (array $arguments, Repeater $component): void {
                ...
                foreach ($items as $item) {
                    if (isset($item['file']) && is_array($item['file']) && !empty($item['file'])) {
                        $mediaUuid = array_keys($item['file'])[0];
                ...
                ->action(function (array $arguments, Repeater $component) {
                    ...
                    if ($media) {
                        try {
                            ...
                            Notification::make()
                                ->title(trans('filament-media-manager::messages.media.notifications.delete-media'))
                                ->success()
                                ->send();
                        } catch (\Exception $e) {
                            Notification::make()
